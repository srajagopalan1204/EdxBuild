...
def extract_text_block(df: pd.DataFrame, text_cols: List[str], drop_values: List[str], joiner: str) -> str:
    ...
    def clean_cell(val: str) -> str:
        # strip and remove lone bad tokens; also collapse internal bad tokens
        t = (val or "").strip()
        # If the whole cell is just a bad token, make it empty
        if BAD_TOKEN_RE.fullmatch(t):
            return ""
        # Otherwise, remove any bad tokens that appear as words
    ...
    def row_ok(row) -> bool:
        # Honor drop_values for any selected text column after cleaning
        for c in text_cols:
            if clean_cell(str(row.get(c, ""))) in drop_values:
                return False
        return True
    ...
    for _, row in df2.iterrows():
        parts = []
        for c in text_cols:
            v = clean_cell(str(row.get(c, "")))
            if v:
                parts.append(v)
        if parts:
            lines.append(" ".join(parts))
    text = (" ".join(lines).strip() if joiner == " " else joiner.join(lines).strip())
    # Final cleanup for any lingering tokens/spaces
...
def main():
    ...
    if rows:
        ...
        # scrub any bad tokens that slipped into merged text
        merged = re.sub(r"\b(?:nan|none|null|na|n/a|nat|-)\b", "", merged, flags=re.I)
    ...
    if not args.no_summarize:
        grade = int(summarize_cfg.get("reading_grade", 8))
simple_grade = int(summarize_cfg.get("simple_grade", 5))
k5_len = int(summarize_cfg.get("simple_max_len", 14))
df_out["Step_narr_out"] = df_out["Step_narr_in"].fillna("").apply(lambda t: summarize_to_grade(t, grade, max_sentence_len, k5_len))
df_out["Step_narr_out_simple"] = df_out["Step_narr_in"].fillna("").apply(lambda t: summarize_to_grade(t, simple_grade, max_sentence_len, k5_len))
        if "Step_narr_m_in" in df_out.columns:
            df_out["Step_narr_m_out"] = df_out["Step_narr_m_in"].fillna("").apply(lambda t: summarize_to_grade(t, grade, max_sentence_len, k5_len))
df_out["Step_narr_m_out_simple"] = df_out["Step_narr_m_in"].fillna("").apply(lambda t: summarize_to_grade(t, simple_grade, max_sentence_len, k5_len))
    # bullets for simple columns if requested
    if bool(summarize_cfg.get("simple_bullets", False)):
        def to_bullets(txt: str) -> str:
            if not txt: return ""
            parts = [p.strip() for p in re.split(r"[.!?]+", txt) if p.strip()]
            return "\n".join(f"- {p}" for p in parts)
        df_out["Step_narr_out_simple"] = df_out["Step_narr_out_simple"].apply(to_bullets)
        if "Step_narr_m_out_simple" in df_out.columns:
            df_out["Step_narr_m_out_simple"] = df_out["Step_narr_m_out_simple"].apply(to_bullets)
...
def simplify_text_k5(text: str, max_len: int = 14) -> str:
    ...
    for a, b in swaps.items():
        text = _re.sub(rf"\b{a}\b", b, text, flags=_re.I)
...
def summarize_to_grade(text: str, grade: int, max_len: int, k5_len: int) -> str:
    if grade <= 5:
        return simplify_text_k5(text, max_len=k5_len)
    else:
        return simplify_text(text, max_len=max_len)
